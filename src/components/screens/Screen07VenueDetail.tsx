...
import { ArrowLeft, Bell, BellOff, Car, Clock, Copy, Expand, ExternalLink, Footprints, Globe, Heart, MapPin, Navigation, Share, Star, ThumbsUp, Ticket, X } from "lucide-react";
...
export default function Screen07VenueDetail({ venue }: { venue: VenueWithDetails }) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-card pb-28">

      {/* ── Scrollable content ───────────────────────────────────────────────── */}
      <div className="flex-1 overflow-y-auto no-scrollbar bg-white">

        {/* ── Static hero ──────────────────────────────────────────────────── */}
        <div className="h-64 relative overflow-hidden shrink-0" style={{ borderRadius: "0 0 24px 24px" }}>
          {activeImage ? (
            <>
              <img
                src={activeImage}
              ...
              {/* Tap-to-expand hint */}
              <div className="absolute bottom-4 right-4 bg-black/30 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center gap-1.5 pointer-events-none">
          ...
          <div className="flex gap-2 px-6 py-3 overflow-x-auto no-scrollbar">
            {venue.image_urls.map((url) => (
              <img
                ...
                className={cn(
                  "w-16 h-16 rounded-xl object-cover shrink-0 cursor-pointer transition-all active:scale-95",
                  activeImage === url ? "border-[3px] border-[#FF5A5F]" : "border-[3px] border-transparent opacity-70"
                )}
              />
            ))}
          </div>
        ...
        {/* ── Content card ─────────────────────────────────────────────────── */}
        <div className="px-6 -mt-6 relative z-10 pb-6">
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    {venue.venue_prices.map((price) => (
                      <div key={price.id} className="flex items-center justify-between rounded-xl border border-border bg-background px-4 py-3">
            ...
            >
              {venue.image_urls?.map((url) => (
                <img
                  ...
                  onClick={() => setActiveImage(url)}
                  className={cn(
                    "w-20 h-20 rounded-xl object-cover shrink-0 cursor-pointer transition-all active:scale-95",
                    activeImage === url
                      ? "border-[3px] border-white"
                      : "border-[3px] border-transparent opacity-60"
                  )}
                />
              ))}
            </div>
