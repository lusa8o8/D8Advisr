...
export default function Screen07VenueDetail({ venue }: { venue: VenueWithDetails }) {
  ...
  const minHeight  = 256;
  const maxHeight  = typeof window !== "undefined" ? window.innerHeight : 844;

  const heroHeight    = Math.min(minHeight + (scrollY / MAX_SCROLL) * (maxHeight - minHeight), maxHeight);
  const borderRadius  = Math.max(24 - (scrollY / MAX_SCROLL) * 24, 0);
  const imageScale    = 1 + (scrollY / MAX_SCROLL) * 0.1;
  const contentOpacity = Math.max(1 - (scrollY / MAX_SCROLL) * 2, 0);
  const isExpanded    = scrollY >= MAX_SCROLL;
    ...
    <div className="flex flex-col h-screen bg-white overflow-hidden">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar bg-white">
        ...
        >
          {activeImage ? (
            <div className="w-full h-full relative">
              ...
              <div
                ...
                style={{ opacity: isExpanded ? 0.7 : 0.4 }}
              />
            </div>
          ) : (
            <div
              className={cn(
                "w-full h-full bg-gradient-to-br flex items-center justify-center text-[80px] opacity-60",
                categoryGradient(venue.category)
              )}
            >
              {heroEmoji}
            </div>
          ...
          <div className="absolute top-14 left-6 right-6 flex justify-between items-center z-20">
            <button
              onClick={() => router.back()}
          ...
          {/* Venue name overlay — fades in when expanded */}
          {isExpanded && (
            <div
              className="absolute bottom-8 left-6 right-6 z-20"
              style={{ opacity: Math.min((scrollY - MAX_SCROLL) / 50, 1) }}
            >
              <p className="text-white text-2xl font-bold leading-tight drop-shadow-lg">{venue.name}</p>
              <p className="text-white/80 text-sm font-medium mt-1 capitalize">{venue.category}</p>
            </div>
          )}

          {/* Scroll hint — drag handle dot */}
          {scrollY < 20 && activeImage && (
        ...
        {venue.image_urls && venue.image_urls.length > 1 && (
          <div
            className="flex gap-2 px-6 py-3 overflow-x-auto no-scrollbar relative z-20 transition-opacity duration-200"
        ...
        {/* ── Content card ─────────────────────────────────────────────────── */}
        <div
          className="relative z-20 bg-white -mt-6 rounded-t-3xl"
          style={{
      ...
      <div
        className="fixed bottom-0 left-0 right-0 bg-card border-t border-border p-6 flex gap-4 z-30 shadow-[0_-10px_20px_rgba(0,0,0,0.03)] transition-opacity duration-200"
      ...
      >
        ...
        <button
          ...
          onClick={() => router.push(`/plans/generate?venue_id=${venue.id}&venue_name=${encodeURIComponent(venue.name)}&venue_category=${encodeURIComponent(venue.category ?? "")}`)}
        >
          Add to Plan
        </button>
      </div>

      {/* ── "Scroll up to close" hint when fullscreen ─────────────────────────── */}
      {isExpanded && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center gap-2 pointer-events-none">
          <div className="bg-white/20 backdrop-blur-md px-4 py-2 rounded-full">
            <p className="text-white text-xs font-bold">Scroll up to close</p>
          </div>
        </div>
      )}
    </div>
