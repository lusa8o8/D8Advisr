...
working with the D8Advisr codebase.
...
- NEVER modify src/middleware.ts
...
- src/app/(main)/ — protected routes: /home, /plans, /profile, /budget, /map, /notifications
- src/app/(admin)/ — admin only: /curator
- src/app/api/ — API routes (server-side only)

### Middleware
Protects: /home, /plans, /profile, /budget, /map,
...
src/app/(main)/[route]/page.tsx → server component
  ...
  passes as props to →
src/components/screens/Screen##Name.tsx → client component
  handles UI and interactions

### Supabase Clients
- src/lib/supabase/server.ts → server components + API routes
- src/lib/supabase/client.ts → client components (supabaseBrowserClient)
...
user_preferences: id, user_id, budget_preference,
                  activity_preferences, default_vibe,
                  group_size_preference, last_updated

experience_logs: id, user_id, plan_id, actual_cost, currency,
                 overall_rating, notes, highlights, created_at

experience_venue_feedback: id, experience_id, venue_id,
                           actual_price, venue_quality_rating,
                           value_rating, vibe_rating,
...
Sizes: 10px, 11px, 12px, 14px, 15px, 16px, 17px,
       22px, 24px, 26px, 28px, 32px
...
## Screen Inventory (26 pages)
01 Welcome          /
02 Auth             /signup, /login
03 Preferences      /onboarding/preferences
04 Home             /home
05 Map              /map
06 Filter Modal     (component on /home)
07 Venue Detail     /venues/[id]
08 Plan Generator   /plans/generate
...
- Sinking fund card on budget page (needs schema)
- Badges system on profile (needs schema)
- Latitude 15° not showing in home feed (confidence_score issue)
- Map venue pins use hardcoded positions
